...
function FieldSeparator({
  ...
  return <div data:slot="field-separator" data:content={!!children} className={cn("relative -my-2 h-5 text-sm group-data-[variant=outline]/field-group:-mb-2", className)} {...props}><Separator className="absolute inset-0 top-1/2" />{children && <span className="bg-background text-muted-foreground relative mx-auto block w-fit px-2" data:slot="field-separator-content">{children}</span>}</div>;
}
function FieldError({
  className,
  children,
  ...
  const content = useMemo(() => {
    ...
    return <ul className="ml-4 flex list-disc flex-col gap-1">{errors.map((error, index) => error?.message && <li>{error.message}</li>)}</ul>;
  }, [children, errors]);
  if (!content) {
    return null;
  }
